...
function addAqiData() {
	...
	air=document.getElementById("aqi-value-input").value.trim();
	//对城市名称的输入进行校验
	if(city==""){
		alert("城市不能为空~");
...
function init() {
  ...
  document.getElementById("aqi-table").addEventListener("click",function(e){
  	if(e.target.className.indexOf('del') > -1)//选中的按钮存在
  	{
  		var vCity=e.target.parentNode.previousSibling.previousSibling.firstChild.nodeValue;
  		delBtnHandle(vCity);
  	}
    //e是被选中的删除按钮，e.target.parentNode是整个aqi-table表格，e.target.parentNode.previousSibling.previousSibling是div
  });
}
